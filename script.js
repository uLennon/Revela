...
(() => {
  ...
  function render(state) {
    ...
    $("ovrTxt").textContent   = Number(state.overround).toFixed(2);
    $("sumProb").textContent  = Number(state.sumProb).toFixed(2);
  ...
  async function vote(pick) {
    ...
    try {
      ...
      $("statusTxt").textContent = "✅ Voto registrado. Obrigado!";
      ["bet1", "betx", "bet2"].forEach(id => $(id).disabled = true);
    } catch (err) {
      console.error("Vote error:", err);
      $("statusTxt").textContent = "❌ Este token já foi usado.";
      ["bet1", "betx", "bet2"].forEach(id => $(id).disabled = true);
    }
  }
